...
(define (how-sema-works)
  (pslide #:title "Key Structure for Sematic Code Completion"
          #:comments ("And the solution boils down to several structures used in Semantic modules. The most important one is CodeCompletionContext "
                      ...
                      "furthermore, we can use the method getBaseType to get the type of the expressions before the dot. Like in this case once we know the expression before the dot is a Car instance, it is quite easy to list all its public members")
          #:go (coord 0.15 0.15 'lt)
          (current-gap-size)
          (tt "CodeCompletionContext")
          (item (t "CodeCompletionContext::getKind() shows the context kind"))
          (annotate (repl-input "f█") (code-block "CCC_TopLevelOrExpression"))
          (annotate (repl-input "car.█") (code-block "CCC_DotMemberAccess"))
          (item (t "CodeCompletionContext::getPreferedType() reveals the type w.r.t the current cursor position"))
